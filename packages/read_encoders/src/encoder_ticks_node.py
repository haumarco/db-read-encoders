...
class MyNode(DTROS):
	...
	def run(self):
		...
		while not rospy.is_shutdown():
			...
			self.left_ticks += (self.cb_left.tally() - self.lastcb_left) * np.sign(self.wheel_dir_left)
			self.right_ticks += (self.cb_right.tally() - self.lastcb_right) * np.sign(self.wheel_dir_right)
			self.lastcb_left = self.cb_left.tally()
			self.lastcb_right = self.cb_right.tally()

			if self.left_ticks != self.last_left_ticks or self.right_ticks != self.last_right_ticks:
